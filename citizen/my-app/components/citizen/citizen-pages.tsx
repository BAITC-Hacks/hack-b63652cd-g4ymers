...
export function CitizenSupportPage() {
  ...
  return <><CitizenPageHeader eyebrow="ПОДДЕРЖКА" title="Как пользоваться приложением" description="Короткая инструкция по безопасному обращению в акимат." /><CitizenTourHelp /><section className="support-hero"><div className="support-hero-icon"><CitizenIcon name="shield" size={24} /></div><div><h2>Ваши данные под защитой</h2><p>Геолокация нужна только для подтверждения места. Активные обращения других жителей вам не показываются.</p></div></section><section className="support-steps"><div className="section-heading"><div><span className="citizen-eyebrow">ИНСТРУКЦИЯ</span><h2>Три шага до обращения</h2></div></div><div className="support-step"><span>1</span><div><strong>Разрешите геолокацию</strong><p>Приложение проверит, что вы действительно рядом с нужной улицей.</p></div></div><div className="support-step"><span>2</span><div><strong>Отсканируйте QR-код</strong><p>Откроются история места, изменения акимата и кнопка «Обратиться в акимат».</p></div></div><div className="support-step"><span>3</span><div><strong>Опишите ситуацию</strong><p>Добавьте описание и срочность. Фото пока недоступны. Активное обращение видите вы и акимат.</p></div></div><Link className="citizen-button primary centered" href="/citizen/scan"><CitizenIcon name="qr" size={17} /> Перейти к сканированию</Link></section><section className="support-faq"><div className="section-heading"><div><span className="citizen-eyebrow">ЧАСТЫЕ ВОПРОСЫ</span><h2>Нужна помощь?</h2></div></div>{questions.map(([question, answer], index) => <div className={`support-question ${openQuestion === index ? "open" : ""}`} key={question}><button onClick={() => setOpenQuestion(openQuestion === index ? null : index)}><strong>{question}</strong><CitizenIcon name="chevron" size={17} /></button>{openQuestion === index && <p>{answer}</p>}</div>)}</section><section className="support-contact"><CitizenIcon name="send" size={19} /><div><strong>Нужно обратиться в акимат?</strong><p>Отсканируйте табличку нужного места и опишите ситуацию.</p></div><a href="/citizen/scan">Сканировать</a></section></>;
}
