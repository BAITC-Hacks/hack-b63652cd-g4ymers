...
import { CitizenTourHelp } from "./onboarding";
import {
  categoryLabels, categoryTone, citizenApi, citizenReportsChanged, formatDate, getMergedComments, statusLabels, statusTone, timeAgo,
  type CitizenCategory, type CitizenProblem, type CitizenProfile,
} from "@/lib/citizen";

function useReportList(load: () => Promise<CitizenProblem[]>) {
  ...
  useEffect(() => {
    let active = true;
    load().then(rows => { if (active) { setProblems(rows); setError(false); } })
  ...
  useEffect(() => {
    ...
    window.addEventListener(citizenReportsChanged, refresh);
    return () => {
      window.removeEventListener("focus", onVisible);
      document.removeEventListener("visibilitychange", onVisible);
      window.removeEventListener(citizenReportsChanged, refresh);
    };
  }, [refresh]);
  return { problems, loading, error, refresh };
}

function useProblems() { return useReportList(citizenApi.getProblems); }
function useMyReports() {
  return useReportList(citizenApi.getMyReports);
}
...
export { CitizenCityMapPage as CitizenMapPage } from "./city-map-page";
...
export function CitizenReportsPage() {
  const { problems, loading, error, refresh } = useMyReports();
  ...
  return <><CitizenPageHeader eyebrow="ЛИЧНЫЙ РАЗДЕЛ" title="Мои обращения" description="Здесь появляются отправленные заявки и актуальные статусы. Активные обращения видны вам и акимату." /><button className="citizen-button secondary small" onClick={refresh} disabled={loading}>Обновить список</button><div className="report-filter filter-scroll"><button className={filter === "ALL" ? "selected" : ""} onClick={() => setFilter("ALL")}>Все</button><button className={filter === "NEW" ? "selected" : ""} onClick={() => setFilter("NEW")}>Новые</button><button className={filter === "IN_PROGRESS" ? "selected" : ""} onClick={() => setFilter("IN_PROGRESS")}>В работе</button><button className={filter === "RESOLVED" ? "selected" : ""} onClick={() => setFilter("RESOLVED")}>Решённые</button></div>{error ? <ErrorState /> : loading ? <LoadingCards /> : filtered.length ? <div className="my-report-list">{filtered.map((problem) => <Link className="my-report-card" href={`/citizen/problems/${problem.id}?private=1`} key={problem.id}><div className="my-report-heading"><CategoryMark category={problem.category} /><div><h2>{problem.title}</h2><span>{categoryLabels[problem.category]} · {problem.district}</span></div><span className={`status-pill ${statusTone[problem.status]}`}><i />{statusLabels[problem.status]}</span></div><StatusTimeline status={problem.status} /><footer><span>Создано: {formatDate(problem.createdAt)}</span><span>№ {problem.id.slice(-6).toUpperCase()} <CitizenIcon name="arrow" size={14} /></span></footer></Link>)}</div> : <EmptyState text={filter === "ALL" ? "Вы пока не отправляли обращения" : "Обращений с таким статусом пока нет"} />}<Link className="citizen-button primary centered" href="/citizen/map"><CitizenIcon name="plus" size={18} /> Сообщить о проблеме</Link></>;
}
