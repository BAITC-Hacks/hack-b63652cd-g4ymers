...
[Проверить за 90 секунд](#demo) · [Скриншоты](#screenshots) · [Запустить локально](#start) · [Деплой на сервер](docs/DEPLOYMENT.md) · [Что работает](#status)

</div>

![Работающий E-AkimAI: карта Астаны, пять выбранных инициатив и предварительный Score 56.54.](docs/screenshots/akim-map.png)

<p align="center"><strong>Один город. Два кабинета. Общая база решений и обращений.</strong><br />Реальный экран приложения: готовый пример из задания, 95 из 100 единиц бюджета, Score 52.56 → 56.54.</p>
...
Скриншоты сделаны в работающем приложении. Нажмите на изображение, чтобы рассмотреть интерфейс.

### Аким: пять решений и понятная проверка плана

[![Проверка сценария: выбранные инициативы, бюджет и предварительный Score перед сохранением.](docs/screenshots/akim-scenario.png)](docs/screenshots/akim-scenario.png)

### Житель: обращение начинается с точки на карте

[![Карта жителя: выбран Байтерек в районе Есиль, место и адрес подтверждаются перед описанием проблемы.](docs/screenshots/citizen-map.png)](docs/screenshots/citizen-map.png)

**Место → описание → проверка → отправка.** QR-код необязателен. После отправки заявка появляется в личном списке и кабинете акима, а рейтинг района учитывает новую проблему.

<details>
<summary><strong>Гайд внутри кабинета акима</strong></summary>

[![Встроенный гайд: сценарий, обращения, рейтинги, AI-советы и управление картой.](docs/screenshots/akim-guide.png)](docs/screenshots/akim-guide.png)

Кнопка **«Как пользоваться»** открывает пять коротких разделов и позволяет сразу перейти к нужной панели.
...
**[Полный пошаговый гайд → docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)**

Основной путь — **Ubuntu VPS + Docker Compose + Caddy с HTTPS**. На сервере работают PostgreSQL, Java API и два Next.js-приложения. Кабинеты открываются на отдельных поддоменах, а API и база доступны внутри контейнерной сети.

В гайде: подготовка сервера и DNS, установка Docker, заполнение переменных окружения, первый запуск и вход акима, проверка работоспособности, логи, обновление, резервные копии, восстановление и типичные ошибки.

Для хостинга нужны **два работающих Next.js-сервера**: авторизация использует серверный BFF и HttpOnly-cookie. Статического экспорта недостаточно. В production требуется HTTPS. Локальный `compose.yml` запускает API и БД; полный серверный стек описан отдельно в [`deploy/`](deploy/).
