...
| Кабинет жителя | Вход и регистрация по ИИН, QR-обращения, свои заявки, журнал решений, комментарии и профиль через общий API |
...
Тот же `BACKEND_URL` можно задать в `citizen/my-app/.env.local`. Для камеры и геолокации на телефоне нужен HTTPS; загрузка фотографий пока не реализована.
...
HTTP-тесты фронтенда акима включаются через `AKIM_API_TEST=1`, `AKIM_TEST_EMAIL` и `AKIM_TEST_PASSWORD` для отдельной тестовой учётной записи; необязательный `AKIM_FRONTEND_URL` по умолчанию равен `http://localhost:3000`. Для кабинета жителя — `CITIZEN_API_TEST=1 npm test` из `citizen/my-app`. Запускайте их с отдельным тестовым backend и БД: проверки создают обращения и сценарии.
