...
export default function Home() {
    ...
    const [panel, setPanel] = useState<"catalog" | "plan" | "district" | "metrics" | "advisor" | null>(null);
    ...
    return <div className={`app-shell immersive-shell ${uiHidden ? "interface-hidden" : ""}`}>
      <CityMap selected={selectedDistrict} onSelect={setSelectedDistrict} plan={plan} fullViewport uiHidden={uiHidden}/>
      <main className="map-interface" aria-label="Управление городом" hidden={uiHidden}>
        ...
        <div className="floating-actions"><button className="icon-button" aria-label="Как играть" title="Как играть" onClick={() => openModal("help")}><Icon name="help" size={19}/></button><button className="icon-button" aria-label="Новый сценарий" title="Новый сценарий" disabled={plan.length === 0} onClick={() => openModal("reset")}><Icon name="reset" size={18}/></button></div>
...
<section hidden={panel !== "metrics"} id="overlay-metrics" className="panel floating-panel metrics-overview" aria-label="Показатели районов"><button className="overlay-close icon-button" aria-label="Закрыть показатели" onClick={closePanel}><Icon name="close"/></button><div className="metrics-heading"><h3>Как меняется город</h3><p>Оценки районов до и после ваших решений</p></div>{result.districts.map((d, i) => <button key={d.id} className={`district-comparison ${selectedDistrict === d.id ? "selected" : ""}`} onClick={() => setSelectedDistrict(d.id)}><span>{d.name}<small>{Math.round(d.population * 100)}% населения</small></span><div className="comparison-track"><i style={{ width: `${d.score}%` }}/><b style={{ width: `${baseline.districts[i].score}%` }}/></div><strong>{formatScore(d.score)}<small>было {formatScore(baseline.districts[i].score)}</small></strong></button>)}<div className="comparison-legend"><span /> Исходное значение <span /> Ваш сценарий</div></section>            <section hidden={panel !== "district"} id="overlay-district" className="panel floating-panel district-panel" aria-labelledby="district-title"><button className="overlay-close icon-button" aria-label="Закрыть район" onClick={closePanel}><Icon name="close"/></button><div className="district-heading"><div className="district-heading-left"><span className="district-icon"><Icon name="pin" size={21}/></span><div><div className="district-title-row"><h2 id="district-title">{selected.name}</h2><span className="subtle-badge">{Math.round(selected.population * 100)}% населения</span></div><p>{selected.note}</p></div></div><div className="district-score"><strong>{formatScore(selected.score)}</strong><span>оценка района</span></div></div>
          ...
          <section hidden={panel !== "catalog"} id="overlay-catalog" className="panel floating-panel catalog-panel" aria-labelledby="catalog-title"><button className="overlay-close icon-button" aria-label="Закрыть инициативы" onClick={closePanel}><Icon name="close"/></button>
            ...
            <div className="catalog-notice" role="status" aria-live="polite">{notice}</div><div className="catalog-caption"><span>{category === "all" ? "Все направления" : categories.find(c => c.id === category)!.short}</span><span>{filtered.length} из 14</span></div>
        ...
        <div className="map-data-note"><span className="live-dot"/> Учебная модель · Синтетические показатели</div>
      </main>
